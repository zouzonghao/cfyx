...
func updateManualSettings(w http.ResponseWriter, r *http.Request) {
	...
	if request.ManualMode {
		...
		log.Printf("Manual mode: Applied %d manually configured group IPs.", configuredIPCount(request.ManualIPs))
	} else {
		log.Println("Manual mode: Disabled. Automatic optimization will resume on its next scheduled run.")
	}
	writeManualSettings(w, http.StatusOK)
}
...
func applyManualIPs(manualIPs map[string]string) {
	...
	for host, hostInfo := range config.Current.HostMap {
		...
		log.Printf("Manual mode: Updating DNS for %s (group: %s) to IP %s", host, hostInfo.Group, ip)
...
func configuredIPCount(manualIPs map[string]string) int {
	count := 0
